...
# Fixed TABnet hyperparameters — mid-range defaults to isolate resampling effect.
# TABnet requires RobustScaler (sensitive to feature scale despite batch norm).
TABNET_FIXED = dict(
    n_d=16, n_a=16,
    n_steps=3,
    gamma=1.3,
...
# ─────────────────────────────────────────────
# STEP 3d.1 — Load data (same canonical split as Scripts 02–03a/b/c)
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 3d.1 — Setup (canonical temporal split 2012–2024)")
print("=" * 60)
...
print(f"\nNote: TABnet has no native class_weight support.")
...
print(f"\nFixed TABnet hyperparameters (resampling effect isolated):")
for k, v in TABNET_FIXED.items():
    if k not in ('seed', 'verbose', 'optimizer_fn'):
        print(f"  {k}: {v}")
print(f"  max_epochs={TABNET_MAX_EPOCHS}, patience={TABNET_PATIENCE}, "
      f"batch_size={TABNET_BATCH_SIZE}")


# ─────────────────────────────────────────────
# Helper — carve early-stopping split from training data
# ─────────────────────────────────────────────
def _es_split(X, y, frac=0.10, seed=RANDOM_STATE):
    """Split off `frac` of training data for TABnet early stopping."""
    n = len(X)
...
def fit_evaluate(X_tr, y_tr, X_v, y_v, sampler):
    """
    Resample (if sampler provided) → RobustScale → fit TABnet → evaluate on val.
    Early stopping is done on a held-out 10% of the (possibly resampled) training data,
    NOT on X_v/y_v, to keep the validation set clean for metric reporting.
    ...
    scaler = RobustScaler()
    X_res_sc = scaler.fit_transform(X_res).astype(np.float32)
    X_v_sc   = scaler.transform(X_v).astype(np.float32)
    y_res_int = y_res.astype(int)

    X_fit, y_fit, X_es, y_es = _es_split(X_res_sc, y_res_int)

    model = TabNetClassifier(**TABNET_FIXED)
    model.fit(
        X_fit, y_fit,
        eval_set=[(X_es, y_es)],
        eval_metric=['auc'],
    ...
    y_proba = model.predict_proba(X_v_sc)[:, 1]
...
# ─────────────────────────────────────────────
# STEP 3d.2 — SMOTE sensitivity scan
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3d.2 — SMOTE sensitivity scan (select best sampling_strategy)")
print("=" * 60)
...
for ss in tqdm(SMOTE_SS_GRID, desc="SMOTE sensitivity"):
    ...
    res_s, _ = fit_evaluate(X_train.copy(), y_train.copy(), X_val, y_val, s)
...
# ─────────────────────────────────────────────
# STEP 3d.3 — Five strategies
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print(f"STEP 3d.3 — Five imbalance-handling strategies (ss={best_ss:.2f})")
print("=" * 60)
...
# ─────────────────────────────────────────────
# STEP 3d.4 — Select winning strategy
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3d.4 — Winner selection (argmax Val PR-AUC)")
print("=" * 60)
...
comp_df.to_csv('outputs/table_08d_imbalance_tabnet.csv', index=False)
print("\nTable 08d saved: outputs/table_08d_imbalance_tabnet.csv")
...
# ─────────────────────────────────────────────
# STEP 3d.5 — Figure 09d
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3d.5 — Figure 09d: Four-panel comparison")
print("=" * 60)
...
ax_A.set_title('Panel A: PR-AUC by Strategy\n(primary metric — higher is better)', fontsize=10)
...
fig.suptitle(
    'Class Imbalance Strategy Comparison — TABnet, Validation Set\n'
...
plt.tight_layout()
plt.savefig('outputs/fig_09d_imbalance_tabnet.png', dpi=300, bbox_inches='tight')
plt.close()
print("Figure 09d saved: outputs/fig_09d_imbalance_tabnet.png")

# ─────────────────────────────────────────────
# Persist selected strategy for Script 04_tabnet
# ─────────────────────────────────────────────
sampler_cfg_str = {
...
with open('outputs/selected_imbalance_strategy_tabnet.txt', 'w') as f:
...
print("Selected strategy saved: outputs/selected_imbalance_strategy_tabnet.txt")

print("\n=== Script 03d complete. All outputs saved to outputs/ ===")
print(f"\nSummary:")
...
print(f"\n  This strategy will be applied to TABnet in Script 04_tabnet.")
