...
# Fixed XGBoost hyperparameters — mid-range defaults to isolate resampling effect
...
STRATEGY_COLORS = {
    'S1_NoResample_ScalePosWeight': '#2c7bb6',
...
# ─────────────────────────────────────────────
# STEP 3b.1 — Load data (same canonical split as Scripts 02–03a)
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 3b.1 — Setup (canonical temporal split 2012–2024)")
print("=" * 60)
...
scale_pos_weight_s1 = n_neg_train / n_pos_train  # used for S1 only
...
print(f"\nS1 scale_pos_weight = {scale_pos_weight_s1:.2f}  "
      f"({n_neg_train} neg / {n_pos_train} pos)")
...
print(f"SMOTE k_neighbors={K_NEIGHBORS} confirmed safe "
      f"(n_positives={n_pos_train}, limit={max_k})")

print(f"\nFixed XGBoost hyperparameters (resampling effect isolated):")
...
# Helper
# ─────────────────────────────────────────────
def fit_evaluate(X_tr, y_tr, X_v, y_v, sampler, xgb_kwargs):
    """
    Resample (if sampler provided) → fit XGBoost → evaluate on val.
    No scaling: XGBoost is tree-based and invariant to feature scale.
    ...
    xgb = XGBClassifier(**XGB_FIXED_PARAMS, **xgb_kwargs)
    xgb.fit(X_res, y_res)
    y_proba = xgb.predict_proba(X_v)[:, 1]
...
# ─────────────────────────────────────────────
# STEP 3b.2 — SMOTE sensitivity scan → select best sampling_strategy
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3b.2 — SMOTE sensitivity scan (select best sampling_strategy)")
print("=" * 60)
...
# ─────────────────────────────────────────────
# STEP 3b.3 — Five strategies (built with best_ss)
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print(f"STEP 3b.3 — Five imbalance-handling strategies (ss={best_ss:.2f})")
print("=" * 60)
print(f"  Strategy       : (see below)")
print(f"  scale_pos_weight = {scale_pos_weight_s1:.2f}  (S1 only)")

STRATEGIES = {
    'S1_NoResample_ScalePosWeight': {
        'sampler':    None,
        'xgb_kwargs': {'scale_pos_weight': scale_pos_weight_s1},
        'label':      f'S1: No resampling\n(scale_pos_weight={scale_pos_weight_s1:.1f})',
        'short':      'S1\nNo resample',
    },
    'S2_RandomOverSampler': {
        'sampler':    RandomOverSampler(sampling_strategy=best_ss, random_state=RANDOM_STATE),
        'xgb_kwargs': {'scale_pos_weight': 1.0},
...
# ─────────────────────────────────────────────
# STEP 3b.4 — Select winning strategy
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3b.4 — Winner selection (argmax Val PR-AUC)")
print("=" * 60)
...
comp_df.to_csv('outputs/table_08b_imbalance_xgb.csv', index=False)
print("\nTable 08b saved: outputs/table_08b_imbalance_xgb.csv")
...
# ─────────────────────────────────────────────
# STEP 3b.5 — Figure 09b
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3b.5 — Figure 09b: Four-panel comparison")
print("=" * 60)
...
fig.suptitle(
    'Class Imbalance Strategy Comparison — XGBoost, Validation Set\n'
...
plt.tight_layout()
plt.savefig('outputs/fig_09b_imbalance_xgb.png', dpi=300, bbox_inches='tight')
plt.close()
print("Figure 09b saved: outputs/fig_09b_imbalance_xgb.png")
...
sampler_cfg_str = {
    'S1_NoResample_ScalePosWeight': f'NoResample(scale_pos_weight={scale_pos_weight_s1:.2f})',
...
with open('outputs/selected_imbalance_strategy_xgb.txt', 'w') as f:
    ...
    f.write(f"scale_pos_weight_s1={scale_pos_weight_s1:.4f}\n")
    f.write(f"best_sampling_strategy={best_ss:.2f}\n")

print("Selected strategy saved: outputs/selected_imbalance_strategy_xgb.txt")

print("\n=== Script 03b complete. All outputs saved to outputs/ ===")
print(f"\nSummary:")
