...
# ─────────────────────────────────────────────
# STEP 7t.1 — Load data, models, and predictions
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 7t.1 — Load data, models, and predictions")
print("=" * 60)
...
# Identify best model from Script 05_tabnet (fallback: TABnet if metrics not found)
try:
    metrics_df = pd.read_csv(find_output('table_10_tabnet_test_metrics.csv'))
    best_model = metrics_df.loc[metrics_df['PR_AUC'].idxmax(), 'Model']
    best_prauc = metrics_df['PR_AUC'].max()
except Exception:
    best_model = 'TABnet'
    best_prauc = np.nan
    print("  (table_10_tabnet not found — defaulting best model to TABnet)")

print(f"\n  Best model: {best_model}  "
      f"(Test PR-AUC = {best_prauc:.4f}, lift = {best_prauc/NO_SKILL_PRAUC:.2f}x)")

# Best-model threshold: argmax F1 on validation set
...
# ─────────────────────────────────────────────
# STEP 7t.2 — Sector-level analysis
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 7t.2 — Sector-level analysis (SIC divisions, test set)")
print("=" * 60)
...
sector_df.to_csv('outputs/table_13_tabnet_sector_analysis.csv', index=False)
print("Table 13_tabnet saved: outputs/table_13_tabnet_sector_analysis.csv\n")
...
ax_a.barh(plot_df['SIC_Division'], plot_df['Base_Rate_pct'],
          color=COLORS[best_model], alpha=0.75)
...
ax_a.set_title(f'Panel A: M&A Target Rate by SIC Division\n(Test Set 2023–2024)', fontsize=10)
...
fig17.suptitle(f'Figure 17_tabnet: Sector-Level Analysis — Test Set (2023–2024)\n'
               f'Best model: {best_model}', fontsize=12)
plt.tight_layout()
plt.savefig('outputs/fig_17_tabnet_sector_analysis.png', dpi=300, bbox_inches='tight')
plt.close()
print("\nFigure 17_tabnet saved: outputs/fig_17_tabnet_sector_analysis.png")


# ─────────────────────────────────────────────
# STEP 7t.3 — Error characterisation
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 7t.3 — Error characterisation: feature profiles for TP / FP / FN")
print("=" * 60)
...
error_df.to_csv('outputs/table_14_tabnet_error_profile.csv', index=False)
print("Table 14_tabnet saved: outputs/table_14_tabnet_error_profile.csv")
print(f"\n  Group sizes:  TP={tp}  FP={fp}  FN={fn}  TN={tn}")

# ── Figure 18_tabnet: heatmap + comparison bar
print("\n  Generating Figure 18_tabnet ...")
...
axes18[0].set_title(f'Panel A: Feature Z-scores by Error Group\n'
...
ax_bar.barh(y_pos + bar_h/2,
            [tp_means[f]      for f in feat_order], height=bar_h,
            color='#d7191c', alpha=0.8, label=f'True Positive (n={tp})')
ax_bar.barh(y_pos - bar_h/2,
            [compare_means[f] for f in feat_order], height=bar_h,
...
fig18.suptitle(f'Figure 18_tabnet: Error Characterisation — Test Set (2023–2024)\n'
               f'Best model: {best_model}', fontsize=12)
plt.tight_layout()
plt.savefig('outputs/fig_18_tabnet_error_characterization.png', dpi=300, bbox_inches='tight')
plt.close()
print("Figure 18_tabnet saved: outputs/fig_18_tabnet_error_characterization.png")


# ─────────────────────────────────────────────
# STEP 7t.4 — Top-50 predicted targets
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 7t.4 — Top-50 predicted M&A targets (Table 15_tabnet)")
print("=" * 60)
...
top50_df.to_csv('outputs/table_15_tabnet_top50_predicted_targets.csv', index=False)
print("Table 15_tabnet saved: outputs/table_15_tabnet_top50_predicted_targets.csv")
...
# ─────────────────────────────────────────────
# STEP 7t.5 — Calibration (Figure 19_tabnet, all 4 models)
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 7t.5 — Probability calibration (Figure 19_tabnet, all 4 models)")
print("=" * 60)
...
for name in MODEL_ORDER:
    ...
    ax_cal.plot(mean_pred, frac_pos, marker='o', lw=lw,
                color=COLORS[name],
    ...
    for b_idx in range(N_BINS):
        ...
        if mask_b.sum() > 0:
            ece += mask_b.sum() / len(proba) * abs(y_test[mask_b].mean() - proba[mask_b].mean())
...
for name in MODEL_ORDER:
    ax_hist.hist(test_probas[name], bins=40, alpha=0.35,
...
fig19.suptitle(f'Figure 19_tabnet: Probability Calibration — Test Set (2023–2024)\n'
               f'All 4 models | Best model: {best_model}', fontsize=12)
plt.tight_layout()
plt.savefig('outputs/fig_19_tabnet_calibration.png', dpi=300, bbox_inches='tight')
plt.close()
print("Figure 19_tabnet saved: outputs/fig_19_tabnet_calibration.png")


# ─────────────────────────────────────────────
# STEP 7t.6 — Threshold sensitivity (Figure 20_tabnet, all 4 models)
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 7t.6 — Threshold sensitivity (Figure 20_tabnet, all 4 models)")
print("=" * 60)

fig20, axes20 = plt.subplots(1, 2, figsize=(14, 5))

linestyles = {'Logistic Regression': '--', 'XGBoost': '--', 'MLP': '--', 'TABnet': '-'}
linewidths  = {'Logistic Regression': 1.2, 'XGBoost': 1.2, 'MLP': 1.2, 'TABnet': 2.5}
...
for name in MODEL_ORDER:
    ...
    axes20[0].plot(THRESHOLD_GRID, f1s,   color=COLORS[name], lw=lw, ls=ls,
                   label=f'{name} F1')
    axes20[0].plot(THRESHOLD_GRID, precs, color=COLORS[name], lw=lw*0.6,
                   linestyle=':', alpha=0.6)
    axes20[0].plot(THRESHOLD_GRID, recs,  color=COLORS[name], lw=lw*0.6,
                   linestyle='-.', alpha=0.6)
    axes20[1].plot(THRESHOLD_GRID, lifts, color=COLORS[name], lw=lw, ls=ls, label=name)
...
axes20[0].set_title('Panel A: F1 (solid/dash), Precision (dotted),\nRecall (dash-dot) vs Threshold',
                    fontsize=10)
axes20[0].legend(fontsize=7.5, loc='center right')
...
fig20.suptitle(f'Figure 20_tabnet: Threshold Sensitivity — Test Set (2023–2024)\n'
               f'All 4 models | Best model: {best_model}', fontsize=12)
plt.tight_layout()
plt.savefig('outputs/fig_20_tabnet_threshold_sensitivity.png', dpi=300, bbox_inches='tight')
plt.close()
print("Figure 20_tabnet saved: outputs/fig_20_tabnet_threshold_sensitivity.png")
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("Script 07_tabnet complete. All outputs saved to outputs/")
print("=" * 60)
...
print(f"\n  Outputs:")
print(f"    table_13_tabnet_sector_analysis.csv")
print(f"    table_14_tabnet_error_profile.csv")
print(f"    table_15_tabnet_top50_predicted_targets.csv")
print(f"    fig_17_tabnet_sector_analysis.png")
print(f"    fig_18_tabnet_error_characterization.png")
print(f"    fig_19_tabnet_calibration.png")
print(f"    fig_20_tabnet_threshold_sensitivity.png")
