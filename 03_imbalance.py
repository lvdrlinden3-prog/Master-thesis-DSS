...
# ─────────────────────────────────────────────
# STEP 3.1 — Load data (same canonical split as Script 02)
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 3.1 — Setup (canonical temporal split 2012–2024)")
print("=" * 60)
...
# ─────────────────────────────────────────────
# STEP 3.2 — Five strategies
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3.2 — Five imbalance-handling strategies")
print("=" * 60)

STRATEGIES = {
    'S1_NoResample_Balanced': {
        'sampler':   None,
    ...
    'S2_RandomOverSampler': {
        'sampler':   RandomOverSampler(sampling_strategy=0.20, random_state=RANDOM_STATE),
        'lr_kwargs': {'class_weight': None},
        'label':     'S2: RandomOverSampler\n(ss=0.20)',
        'short':     'S2\nRandOver',
    },
    'S3_RandomUnderSampler': {
        'sampler':   RandomUnderSampler(sampling_strategy=0.20, random_state=RANDOM_STATE),
        'lr_kwargs': {'class_weight': None},
        'label':     'S3: RandomUnderSampler\n(ss=0.20)',
        'short':     'S3\nRandUnder',
    },
    'S4_SMOTE': {
        'sampler':   SMOTE(k_neighbors=K_NEIGHBORS, sampling_strategy=0.20,
                           random_state=RANDOM_STATE),
        'lr_kwargs': {'class_weight': None},
        'label':     f'S4: SMOTE\n(k={K_NEIGHBORS}, ss=0.20)',
        'short':     'S4\nSMOTE',
    },
    'S5_SMOTEENN': {
        'sampler':   SMOTEENN(
                         smote=SMOTE(k_neighbors=K_NEIGHBORS, sampling_strategy=0.20,
                                     random_state=RANDOM_STATE),
                         random_state=RANDOM_STATE
                     ),
        'lr_kwargs': {'class_weight': None},
...
def fit_evaluate(X_tr, y_tr, X_v, y_v, sampler, lr_kwargs):
    """
    Resample (if sampler provided) → RobustScale → fit LR → evaluate on val.
    Scaler is fitted on the (potentially resampled) training data.
    ...
    y_proba = lr.predict_proba(X_v_sc)[:, 1]
...
# SMOTE sensitivity sub-test (Strategy 4 only)
print("\n" + "-" * 60)
print("  SMOTE sensitivity sub-test (S4 only — sampling_strategy: 0.10 / 0.20 / 0.50)")
print("-" * 60)

smote_sensitivity = {}
for ss in tqdm([0.10, 0.20, 0.50], desc="SMOTE sensitivity"):
...
# ─────────────────────────────────────────────
# STEP 3.3 — Select winning strategy
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3.3 — Winner selection (argmax Val PR-AUC)")
print("=" * 60)
...
comp_df.to_csv('outputs/table_08_imbalance_comparison.csv', index=False)
print("\nTable 08 saved: outputs/table_08_imbalance_comparison.csv")
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3.4 — Figure 09: Four-panel comparison")
print("=" * 60)
...
ax_C.set_xlim([0, 1])
ax_C.set_ylim([0, 0.25])   # compress y to show curve differences clearly
ax_C.grid(alpha=0.3)
...
ax_D.set_xticklabels([f'ss={ss:.2f}' for ss in ss_vals], fontsize=9)
ax_D.set_ylabel('Metric value')
ax_D.set_title(f'Panel D: SMOTE Sensitivity\n(S4, k={K_NEIGHBORS}, sampling_strategy)',
               fontsize=10)
...
fig.suptitle(
    'Class Imbalance Strategy Comparison — Logistic Regression, Validation Set\n'
...
plt.tight_layout()
plt.savefig('outputs/fig_09_imbalance_comparison.png', dpi=300, bbox_inches='tight')
plt.close()
print("Figure 09 saved: outputs/fig_09_imbalance_comparison.png")

# ─────────────────────────────────────────────
# Persist selected strategy for Script 04
# ─────────────────────────────────────────────
SAMPLER_CONFIGS = {
    'S1_NoResample_Balanced': 'NoResample',
    'S2_RandomOverSampler':   'RandomOverSampler(sampling_strategy=0.20)',
    'S3_RandomUnderSampler':  'RandomUnderSampler(sampling_strategy=0.20)',
    'S4_SMOTE':               f'SMOTE(k_neighbors={K_NEIGHBORS}, sampling_strategy=0.20)',
    'S5_SMOTEENN':            f'SMOTEENN(SMOTE(k_neighbors={K_NEIGHBORS}, sampling_strategy=0.20))',
}

with open('outputs/selected_imbalance_strategy.txt', 'w') as f:
    f.write(f"selected_strategy={best_name}\n")
    f.write(f"sampler_config={SAMPLER_CONFIGS[best_name]}\n")
...
print("Selected strategy saved: outputs/selected_imbalance_strategy.txt")
...
print(f"  Val ROC-AUC: {best_res['ROC_AUC']:.4f}")
print(f"  Val F1     : {best_res['F1']:.4f}  (threshold = {best_res['Threshold']:.3f})")
print(f"\n  This strategy will be applied to ALL models in Script 04.")
